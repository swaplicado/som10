 ...
 */
public class SViewWarehouseFillLevel extends SGridPaneView {

    public SViewWarehouseFillLevel(SGuiClient client, String title) {
        super(client, SGridConsts.GRID_PANE_VIEW, SModConsts.SU_WAH_FILL_LEVEL, SLibConsts.UNDEFINED, title);
    }
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT "
                + "v.id_wah_fill_level AS " + SDbConsts.FIELD_ID + "1, "
                ...
                + "FROM " + SModConsts.TablesMap.get(SModConsts.SU_WAH_FILL_LEVEL) + " AS v "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.CU_USR) + " AS ui ON "
                + "v.fk_usr_ins = ui.id_usr "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.CU_USR) + " AS uu ON "
                + "v.fk_usr_upd = uu.id_usr "
                + (sql.isEmpty() ? "" : "WHERE " + sql)
                + "ORDER BY v.name, v.val_min, v.val_max ";
    }
    ...
    public void createGridColumns() {
        ...
        columns[col++] = new SGridColumnView(SGridConsts.COL_TYPE_TEXT_NAME_CAT_M, "color", "Color");
        columns[col++] = new SGridColumnView(SGridConsts.COL_TYPE_DEC_2D, "val_min", "Valor mínimo");
        columns[col++] = new SGridColumnView(SGridConsts.COL_TYPE_DEC_2D, "val_max", "Valor máximo");

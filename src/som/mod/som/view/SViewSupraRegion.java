 ...
 */
public class SViewSupraRegion extends SGridPaneView {

    public SViewSupraRegion(SGuiClient client, String title) {
        super(client, SGridConsts.GRID_PANE_VIEW, SModConsts.SU_SUP_REG, SLibConsts.UNDEFINED, title);
    }
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT "
                + "v.id_sup_reg AS " + SDbConsts.FIELD_ID + "1, "
                ...
                + "FROM " + SModConsts.TablesMap.get(SModConsts.SU_SUP_REG) + " AS v "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.CU_USR) + " AS ui ON "
                + "v.fk_usr_ins = ui.id_usr "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.CU_USR) + " AS uu ON "
                + "v.fk_usr_upd = uu.id_usr "
                + (sql.isEmpty() ? "" : "WHERE " + sql)
                + "ORDER BY v.name, v.id_sup_reg ";
    }
    ...
    public void createGridColumns() {
        ...
        columns[col++] = new SGridColumnView(SGridConsts.COL_TYPE_INT_RAW, "v.sort", "Orden");

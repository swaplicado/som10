...
public class SReportHtmlTicketSeasonMonthAlt {
    ...
    public String generateReportHtml(final String[] itemIdsPairs, final int yearRef, final int intvlDays, final int seasonFirstMonth, final int monthFirstDay, final Date cutoff, final Date now, final int ticketOrigin, final int ticketDestination, final int mode) throws Exception {
        ...
        for (String itemIdsPair : itemIdsPairs) {
            ...
            double[][] tableCnvValues = new double[years + 1][SLibTimeConsts.MONTHS]; // total weight per year, per month
            double[] tableCnvTotals = new double[years + 1]; // total weight per year
            double[] tableCnvAccums = new double[years + 1]; // accumulated weight per year up to today
            
            // for alternative tickets:
            double[][] tableAltValues = new double[years + 1][SLibTimeConsts.MONTHS]; // total weight per year, per month
            double[] tableAltTotals = new double[years + 1]; // total weight per year
            double[] tableAltAccums = new double[years + 1]; // accumulated weight per year up to today
            
            // for total (all) tickets:
            double[][] tableTotValues = new double[years + 1][SLibTimeConsts.MONTHS]; // total weight per year, per month
            double[] tableTotTotals = new double[years + 1]; // total weight per year
            double[] tableTotAccums = new double[years + 1]; // accumulated weight per year up to today
            
            HashMap<Integer, Integer> tableMaxYearTotValuesMap = new HashMap<>(); // key: year index, value: row index
            ...
            for (int row = 0; row < SLibTimeConsts.MONTHS; row++) {
                ...
                for (int col = 0; col < tableTotTotals.length; col++) {
                    ...
                    Integer rowOfMaxTotValue = tableMaxYearTotValuesMap.get(col);
                    if (rowOfMaxTotValue == null || valueTot > tableTotValues[col][rowOfMaxTotValue]) {
                        tableMaxYearTotValuesMap.put(col, row);
                    }
                }
            ...
            for (int row = 0; row < SLibTimeConsts.MONTHS; row++) {
                ...
                for (int col = 0; col < tableTotTotals.length; col++) {
                    boolean isMax = tableMaxYearTotValuesMap.get(col) == row;
